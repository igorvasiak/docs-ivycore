...
* :ref:`GetIndexedPath <string GetIndexedPath(string path, string fileName, string extension, int index)>`
* :ref:`LoadSprite <Sprite LoadSprite(string path, int width, int height)>`
* :ref:`LoadToNullSprite and TryLoadToNullSprite`
* :ref:`LoadTexture <Texture2D LoadTexture(string path, int width, int height)>`
...
* **Texture2D LoadToNullSprite(ref Sprite sprite, string path, int width, int height)** - Loads the image and sets sprite if sprite is null. It returns sprite even if not null.
* **bool TryLoadToNullSprite(ref Sprite sprite, string path, int width, int height)** - Loads the image and sets sprite if sprite is null. It returns wether it set tex or not.
...
* **Texture2D LoadToNullTexture(ref Texture2D tex, string path, int width, int height)** - Loads the texture and sets tex if tex is null. It returns tex even if not null.
* **bool TryLoadToNullTexture(ref Texture2D tex, string path, int width, int height)** - Loads the texture and sets tex if tex is null. It returns wether it set tex or not.
